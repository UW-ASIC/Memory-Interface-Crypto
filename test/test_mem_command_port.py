...
def rd_key_generate():
    ...
    src = random.choice([0b01, 0b10])   # SHA or AES
    dest = 0b00
    opcode = 0b00                       # RD_KEY
...
def invalid():
    enc = random.randint(0,1)
    dest = random.choice([0b01, 0b10, 0b11])  # invalid for MEM
...
async def do_test_rd_key_decode(dut):
    ...
    inputarr = [rd_key_generate(),0xBA, 0xDC, 0xFE]
    for i in inputarr:
    ...
    assert int(dut.length.value) == 256,f"length expecpted 256 got {int(dut.length.value)}"
    # addr
...
async def do_test_rd_text_decode(dut):
    # read text decode: check ena signal, r_w signal, length, address
    await rst(dut)
    ...
    inputarr = [rd_txt_generate(),0xBA, 0xDC, 0xFE]
    for i in inputarr:
        dut.in_bus_data.value = i
        await RisingEdge(dut.clk)
    ...
    # length
    assert dut.length_valid.value == 1,f"length_valid expecpted 1 got {dut.length_valid.value}"
    assert int(dut.length.value) == 128,f"length expecpted 128 got {int(dut.length.value)}"
    # addr
    assert dut.address_valid.value == 1,f"address_valid expecpted 1 got {dut.address_valid.value}"
    assert int(dut.address.value) == 0xfedcba,f"address expecpted 0xfedcba got {int(dut.address.value):#08x}" 
...
async def do_test_wr_res_decode(dut):
    ...
    inputarr = [wr_aes_generate(),0xBA, 0xDC, 0xFE]
    for i in inputarr:
        dut.in_bus_data.value = i
        await RisingEdge(dut.clk)
    ...
    inputarr = [wr_sha_generate(),0x21, 0x43, 0x65]
    for i in inputarr:
        dut.in_bus_data.value = i
        await RisingEdge(dut.clk)
...
async def do_test_read_from_bus(dut):
    ...
    async def rdkey():
        ...
        rd_key = [randomized_data() for _ in range(32)]

        # read key randomized opcode, and address 0xfedcba from 23:0
        rd_keyarr = [rd_key_generate(),0xBA, 0xDC, 0xFE]
    ...
    async def rdtxt():
        ...
        rd_txt = [randomized_data() for _ in range(16)]
        # read text opcode with randomized opcode, and address 0x654321 
        rd_txtarr = [rd_txt_generate(),0x21, 0x43, 0x65]
        ...
        producer_rdtxt = cocotb.start_soon(fsm_cu_rdtxt())
        await cu_bus_rdtxt()
...
async def do_test_write_to_bus(dut):
    ...
    async def wrsha():
        ...
        wr_sha_generate_data = [randomized_data() for _ in range(32)]

        # write from sha randomized opcode, and address 0xfedcba from 23:0
        wr_sha_generatearr = [wr_sha_generate(),0xBA, 0xDC, 0xFE]
        dut.bus_valid.value = 1
        for i in wr_sha_generatearr:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
        ...
        i = 0
        while i < len(wr_sha_generate_data):
            dut.in_bus_data.value = wr_sha_generate_data[i]
        ...
        for j in range(31, -1, -1):
            assert ((dut_val>>8*j)&0xff) == wr_sha_generate_data[j],\
            f"out_fsm_data expect {wr_sha_generate_data[j]} got {((dut_val>>8*j)&0xff):#04x}"

    # aes write
    async def wraes():
        # 16 B/128b randomized data
        wr_aes_generate_data = [randomized_data() for _ in range(16)]

        # write from aes randomized opcode, and address 0x654321 from 23:0
        wr_aes_generatearr = [wr_aes_generate(),0x21, 0x43, 0x65]
        dut.bus_valid.value = 1
        for i in wr_aes_generatearr:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
        ...
        # randomized back pressure from bus
        i = 0
        while i < len(wr_aes_generate_data):
            dut.in_bus_data.value = wr_aes_generate_data[i]
            dut.bus_valid.value = random.randint(0,1)
            await RisingEdge(dut.clk)
        ...
        for j in range(15, -1, -1):
            assert ((dut_val>>8*j)&0xff) == wr_aes_generate_data[j],\
            f"out_fsm_data expect {wr_aes_generate_data[j]} got {((dut_val>>8*j)&0xff):#04x}"
...
async def do_test_fsm_handshake(dut):
    ...
    async def rd_key_ack_flow():
        ...
        # data generate
        data = [randomized_data() for _ in range (32)]
    # read key opcode with randomized opcode, and address 0xfedcba from 23:0
        opcode = [rd_key_generate(),0xBA, 0xDC, 0xFE]
        for i in opcode:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
    ...
    async def rd_txt_ack_flow():
        ...
        # data generate
        data = [randomized_data() for _ in range (16)]
    # read text opcode with randomized opcode, and address 0xfedcba from 23:0
        opcode = [rd_txt_generate(),0xBA, 0xDC, 0xFE]
        for i in opcode:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
    ...
    # write sha key opcode with randomized opcode, and address 0xfedcba from 23:0
        opcode = [wr_sha_generate(),0xBA, 0xDC, 0xFE]
        for i in opcode:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
    ...
    async def wraes_ack_flow():
        ...
        data = [randomized_data() for _ in range (16)]
    # read text opcode with randomized opcode, and address 0xfedcba from 23:0
        opcode = [wr_aes_generate(),0xBA, 0xDC, 0xFE]
        for i in opcode:
            dut.in_bus_data.value = i
            await RisingEdge(dut.clk)
